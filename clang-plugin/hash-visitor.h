...
class CHashVisitor
    : public RecursiveASTVisitor<CHashVisitor> {
    typedef RecursiveASTVisitor<CHashVisitor> Inherited;
...
#define DEF_ADD_DATA(CLASS, CODE)                                       \
    ...
    bool Visit##CLASS(const CLASS *S) {                                 \
        unsigned tag = CHashVisitorPrefix::CLASS;                       \
        if (tag != 0) {                                                 \
            std::cerr << "  " #CLASS << " " << S << "\n";               \
            addData(tag);                                               \
        }                                                               \
...
public:
    ...
    void storeHash(const Decl *Obj, Hash::Digest Dig) {
        DeclSilo[Obj] = Dig;
    }
